...
export const TeamCarousel: React.FC<TeamCarouselProps> = ({
  ...
  return (
    <div
      ...
      className={cn(`w-full py-12 flex flex-col items-center justify-center overflow-hidden relative`, className)}
    ...
    >
      ...
      {title && (
        <h1
          className={cn(
            "font-black uppercase tracking-tight text-center mb-10",
      ...
      >
        ...
        {showArrows && (
          <>
            <motion.button
              onClick={() => paginate(-1)}
              className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-card/60 hover:bg-card/80 text-foreground w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110 shadow-md"
              whileTap={{ scale: 0.9 }}
            >
            ...
            <motion.button
              onClick={() => paginate(1)}
              className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-card/60 hover:bg-card/80 text-foreground w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110 shadow-md"
              whileTap={{ scale: 0.9 }}
            >
            ...
            {members.map((member, index) => {
              ...
              return (
                <motion.div
                  ...
                  onClick={() => {
                    if (!isCurrent) {
                      const newDirection = wrapIndex(index - currentIndex) > totalMembers / 2 ? -1 : 1;
                      if(position.startsWith('left')) paginate(-1);
                      else if(position.startsWith('right')) paginate(1);
                      else {
      ...
      {infoPosition === 'bottom' && members[currentIndex] && (
        ...
        >
          <h2
            className="text-2xl font-bold mb-1 relative inline-block font-headline"
